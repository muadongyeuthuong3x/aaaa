<template>
	...
	<el-menu :unique-opened="true" :router="true" active-text-color="#ff4140" :collapse-transition="true" :default-active="$route.path" :collapse="aside"  mode="vertical">
		<center>
		...
		<el-submenu :index="'/dashboard/san-thuong-mai'" :default-active="$route.path" :router="true">
			...
			<el-menu-item-group title="Tài khoản sàn">
				<el-menu-item v-for="(data,i) in ec" :index="data.path" :key="i" align="left">
					<span>{{data.name}}</span>
					<el-tag align="right" :type="getCount(data.name) > 0 ? 'success' : 'info'" class="tag-ec" :closable="false" effect="light" size="mini" :disable-transitions="false">{{getCount(data.name)}}</el-tag>
				</el-menu-item>
			</el-menu-item-group>
