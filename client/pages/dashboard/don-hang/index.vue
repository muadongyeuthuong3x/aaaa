			...
			<el-row :gutter="20">
				<el-col align="left" :span="24" :offset="0">
						...
						<template slot="label">
							<div>
								...
								<span style="margin-left: 5px">
									...
									<el-tag v-if="item.load_order === false" type="info"effect="plain" size="mini" :disable-transitions="false">{{item.order_count}}</el-tag>
					...
					<el-table :data="orders" :border="true" :highlight-current-row="true" :stripe="false" :lazy="false" :show-summary="false" tooltip-effect="light" style="width: 100%">
						<el-table-column label="Thời gian" align="center" :sortable="false" :fixed="false" width="130">
							<template slot-scope="scope">
								{{TimeConvert(scope.row.time)}}
							</template>
						</el-table-column>
						
						
						<el-table-column label="Order ID" prop="order_id" align="center" :sortable="false" :fixed="false" width="173"/>
...
<script>
	export default {
		...
		methods:{
			...
			async Load(e){
				e.load_order = true;
				let {data} = await this.$axios.post('/api/order/load',{accountec_id:e.id});
