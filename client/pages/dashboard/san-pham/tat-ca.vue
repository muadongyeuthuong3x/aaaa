			...
			<el-row :gutter="20">
				<el-col align="center" :span="24" :offset="0">
					...
					<el-tabs @tab-click="ActiveTab" v-model="tab" type="border-card">
						...
						<el-table row-class-name="test" v-loading="load_data" ref="data" :data="products" max-height="1200" :border="true" :highlight-current-row="true" :stripe="false" :lazy="false" :show-summary="false" tooltip-effect="light" style="width: 100%">
							...
							<el-table-column label="Giá bán" prop="price" align="center" :sortable="false" :fixed="false" width="180">
								<template slot-scope="scope">
									<el-popover trigger="click" placement="top-start" title="Chỉnh giá : " width="150" transition="el-zoom-in-center">
										...
										<el-input :placeholder="scope.row.price" @keyup.enter.native="UpdatePrice(scope.row)" v-model="scope.row.price_new" size="medium" type="text" :autosize="false" :disabled="!scope.row.active" autocomplete="off"></el-input>
										<br> <br>
										<center>
		...
		methods:{
			...
			async Load(e){
				try{
					e.load_status = false;
					e.count = 0;
					let {data} = await this.$axios.post('/api/product/reload',{accountec_id:e.id});
			...
			async UpdatePrice(e){
				...
				try{
					...
					let {price_new,price,accountec_id,product_id,id,quantity} = e;
					let {data} = await this.$axios.post('/api/edit/price',{price_new,accountec_id,product_id});
					if(data.error) throw data.error;
					e.price_new = '';
