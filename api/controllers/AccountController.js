...
module.exports = {
	...
	register:async function(req,res){
		try{
			let {fullname,token_phone,password} = req.body;

			let find_phone = await  Verify.findOne({token_phone})
			if(!find_phone) throw 'Không tìm thấy Số điện thoại khi đăng kí.';
			let phone = find_phone.phone;
			let find_exists = await Account.findOne({phone});
			if(find_exists) throw 'Số điện thoại này đã được sử dụng.';
			...
			await Verify.destroy({token_phone});
			await Account.create({id_user,fullname,phone,pass_hash,permission:permission.filter(({def})=>def).map(({slug})=>slug)});
